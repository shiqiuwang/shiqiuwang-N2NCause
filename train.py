        ...
        for epoch in tqdm(range(main_cfg.num_epochs), desc='Epochs'):
            ...
            for batch in tqdm_iter:
                ...
                if batch % main_cfg.eval_every == 0:
                    ...
                    #pred = session.run(feed_dict=feed_dict_train)
                    log_saver.log_train(train_summary, global_step)
                    
                    #print(train_f1)
                    # f1
                    # train_f1 = f1_score(,train_mini_labels)
                    # if train_f1 > min_f1:
                    #     min_f1 = train_f1
                    # else:
                    #     print()
            ...
            model_saver.save(session, global_step=global_step)
            print('\ndev_f1 = {}, train_f1 = {}, dev_acc = {}, train_acc = {}, loss = {} '.format(dev_f1, train_f1, dev_accuracy, train_accuracy, loss))
...
def predict(
    ...
    model = MODELS[model]
    main_cfg = MainConfig(main_config)
    sentence_pair = ['Wet brown dog swims towards camera.','A dog is in the water.']
    # model_dir = str(main_config['DATA']['model_dir'])
    
    print(main_cfg.model_dir)

    vectorizer = DatasetVectorizer(
        model_dir=main_cfg.model_dir,
        char_embeddings=main_cfg.char_embeddings,
        raw_sentence_pairs=sentence_pair
    )
...
def main():
    ...
    parser.add_argument(
        'dataset',
        choices=['QQP', 'SNLI', 'ANLI', 'Crest','ECSIN','ECMUL','SCISIN','SCIMUL'],
        nargs='?',
